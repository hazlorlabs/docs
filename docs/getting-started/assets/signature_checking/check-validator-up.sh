...
if [[ ! -z "${BECH_PUBKEY}" ]]; then
    check_chain_maind
    if [[ "${RET_VALUE}" != 0 ]]; then
        echoerr "hazlord is not installed or not in PATH. Please install it first or check hazlord is added to PATH for pubkey key conversion."
    fi
    PUBKEY=$(hazlord debug pubkey ${BECH_PUBKEY} 2>&1 | grep "tendermint/PubKeyEd25519" | cut -d : -f2- | jq -r .value || echoerr "Decode Pubkey Failed ❌")
fi
if [[ -z "${PUBKEY}" ]]; then
